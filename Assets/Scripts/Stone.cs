...
public class Stone : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (currentTile.name != startingTile.name) startingTile.SetIsStone(false);
        if (!isFalling)
        {
    ...
    private void OnTriggerEnter2D(Collider2D otherCollider)
    {
            ...
            if (slotComponent != null)
            {
                ...
                if (parentTransform != null)
                {
                    ...
                    if (nextTile != null && nextTile.isFilled() && startingTile)
                    {
                        ...
                        string startingId = startingTile.name;
                        if (startingId != nextTile.name && startingId != currentTile.name)
                        {
        ...
        if (otherCollider.CompareTag("Enemy") && isFalling)
        {
            Enemy enemy = otherCollider.GetComponent<Enemy>();
            if (enemy && enemy.transform.position.y < transform.position.y) // Ensure enemy is below
            {
                enemy.Die();
            }
        }
